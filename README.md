...
- **读书管理**：阅读时长统计、连续打卡、近 7 天柱状图、本月打卡日历、每日阅读提醒（订阅消息）
...
v1 书库与阅读器 → v2 自藏书导入 → v3 智能阅读/统计/打卡 → v4 登录体系 → v5 云同步 → v6 交付校验 → v7-v8 打开失败修复（压缩数据模块）→ v9 多格式导入（PDF/EPUB）→ v10 多端批量上传 → v11 微信/QQ 文件导入指南。详见 `docs/使用说明.md`。