...
- **书库**：内置 26 本公版古籍（四书注疏、道德经、庄子、传习录、菜根谭、唐诗三百首、古文观止……），正文以 deflate+base64 压缩内置（`data/books_data.js`），不依赖网络与文件系统
...
- **摘抄与报告**：书签跨书汇总成「我的摘抄」、段落笔记（写心得，跨书汇总导出）、阅读报告卡（canvas 生成，保存相册分享）、全界面动效
- **读书计划**：内置全年读书计划页（每月主读/古籍日课/随翻书 + 备选书池）
- **足迹与备份**：近 26 周打卡热力图、一键备份/恢复数据（JSON，换设备迁移）
- **书目库**：大学生必读书目 60 本（教育部 100 本 + 清北书单精选，★ 核心 + 书架直达）+ 如何读方法论
- **新手引导**：首次启动 4 屏引导（可跳过、可重看）
- **朗读**：长按段落一键朗读（微信同声传译插件，需在公众平台添加免费插件）
...
| ![计划](screenshots/13_读书计划页.png) | ![书目库](screenshots/14_书目库.png) | | |
|---|---|---|---|
...
v1 书库与阅读器 → v2 自藏书导入 → v3 智能阅读/统计/打卡 → v4 登录体系 → v5 云同步 → v6 交付校验 → v7-v8 打开失败修复（压缩数据模块）→ v9 多格式导入（PDF/EPUB）→ v10 多端批量上传 → v11 微信/QQ 文件导入指南 → v12 阅读体验升级 → v13 摘抄/报告卡 → v14 段落笔记 + 计划页 → v15 蒙学书/足迹热力图/备份 → v16 书目库 → v17 体积优化/朗读/笠翁对韵 → v18 数据恢复/排版增强 → v19 新手引导。详见 `docs/使用说明.md`。