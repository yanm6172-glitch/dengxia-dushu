...
| 书摘卡 | 主题面板 | 护眼主题 | |
|---|---|---|---|
| ![书摘卡](screenshots/09_书摘卡.png) | ![主题](screenshots/10_阅读_主题面板.png) | ![护眼](screenshots/11_阅读_护眼主题.png) | |
|---|---|---|---|
...
v1 书库与阅读器 → v2 自藏书导入 → v3 智能阅读/统计/打卡 → v4 登录体系 → v5 云同步 → v6 交付校验 → v7-v8 打开失败修复（压缩数据模块）→ v9 多格式导入（PDF/EPUB）→ v10 多端批量上传 → v11 微信/QQ 文件导入指南 → v12 阅读体验升级（主题/字体/沉浸/自动滚动/全文搜索/书摘卡/每日目标）。详见 `docs/使用说明.md`。